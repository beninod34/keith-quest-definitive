...
module Transitions
  ...
  def self.directed(reverse = false)
    ...
    for i in 0...(16 * self.delta).round
  ...
  def self.circular(reverse = false)
    ...
    viewport.z = 99999
    zoom = reverse ? 0 : 2.0
    bmp = Sprites::Bitmap.new(Graphics.width,Graphics.height)
    bmp.fill_rect(0,0,bmp.width,bmp.height,Color.new(0,0,0))
    for i in 0...4
      bmp.draw_circle(Color.new(0,0,0,120 - 40*i), radius: bmp.height/2 - 12*i)
    end
    ...
    for i in 0..(20 * self.delta).round
      sprite.bitmap.clear
  ...
  def self.zoom(reverse = false, screen = nil)
    ...
    if !screen
      viewport = Viewport.new(0,0,Graphics.width,Graphics.height)
      viewport.z = 99999
      screen = Sprites::Base.new(viewport)
      screen.snap_screen
      screen.center!
      screen.x = Graphics.width/2
      screen.y = Graphics.height/2
    end
    ((reverse ? 32 : 48) * self.delta).round.times do
